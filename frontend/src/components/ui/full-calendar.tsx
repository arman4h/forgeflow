...
const dayEventVariants = cva("font-bold border-l-4 rounded p-2 text-xs", {
  variants: {
    variant: {
      default: "bg-zinc-100 dark:bg-zinc-900 text-zinc-700 dark:text-zinc-300 border-zinc-400 dark:border-zinc-500",
      blue: "bg-blue-500/20 text-blue-600 dark:text-blue-400 border-blue-500",
      green: "bg-emerald-500/20 text-emerald-600 dark:text-emerald-400 border-emerald-500",
      pink: "bg-pink-500/20 text-pink-600 dark:text-pink-400 border-pink-500",
      purple: "bg-violet-500/20 text-violet-600 dark:text-violet-400 border-violet-500",
    },
  },
  defaultVariants: {
    variant: "default",
...
const EventGroup = ({
  ...
  return (
    <div className="h-20 border-t border-zinc-200 dark:border-zinc-800 last:border-b">
      {events
...
const CalendarWeekView = () => {
  ...
  return (
    <div className="flex flex-col relative overflow-auto h-full">
      <div className="flex sticky top-0 bg-white dark:bg-black z-10 border-b border-zinc-200 dark:border-zinc-800 mb-3">
        <div className="w-12"></div>
        {headerDays.map((date, i) => (
          <div
            key={date.toString()}
            className={cn(
              "text-center flex-1 gap-1 pb-2 text-sm text-zinc-500 dark:text-zinc-400 flex items-center justify-center",
              [0, 6].includes(i) && "text-zinc-400 dark:text-zinc-500",
            )}
          >
            ...
            <span
              className={cn(
                ...
                isToday(date) &&
                  "bg-indigo-600 text-white dark:bg-indigo-500 rounded-full size-6",
              )}
            >
              {format(date, "d")}
            </span>
        ...
        <div className="grid grid-cols-7 flex-1">
          {weekDates.map((hours, i) => {
            return (
              <div
                className={cn(
                  "h-full text-sm text-zinc-500 dark:text-zinc-400 border-l border-zinc-200 dark:border-zinc-800 first:border-l-0",
                  [0, 6].includes(i) && "bg-zinc-50 dark:bg-zinc-950",
                )}
...
const CalendarMonthView = () => {
  ...
  return (
    <div className="h-full flex flex-col">
      <div className="grid grid-cols-7 gap-px sticky top-0 bg-white dark:bg-black border-b border-zinc-200 dark:border-zinc-800">
        {weekDays.map((day, i) => (
          <div
            key={day}
            className={cn(
              "mb-2 text-right text-sm text-zinc-500 dark:text-zinc-400 pr-2",
              [0, 6].includes(i) && "text-zinc-400 dark:text-zinc-500",
            )}
          >
            {day}
      ...
      <div className="grid overflow-hidden -mt-px flex-1 auto-rows-fr p-px grid-cols-7 gap-px">
        {monthDates.map((_date) => {
          ...
          return (
            <div
              className={cn(
                "ring-1 ring-zinc-200 dark:ring-zinc-800 p-2 text-sm text-zinc-500 dark:text-zinc-400 overflow-auto",
                !isSameMonth(date, _date) && "text-zinc-300 dark:text-zinc-600",
              )}
              key={_date.toString()}
            >
              <span
                className={cn(
                  ...
                  isToday(_date) && "bg-indigo-600 text-white dark:bg-indigo-500",
                )}
              >
                {format(_date, "d")}
              </span>
              ...
              {currentEvents.map((event) => {
                return (
                  ...
                  >
                    ...
                    <time className="tabular-nums text-zinc-400 dark:text-zinc-500 text-xs">
    ...
    <div className="grid grid-cols-4 gap-10 overflow-auto h-full">
      {months.map((days, i) => (
        <div key={days[0].toString()}>
          <span className="text-xl text-zinc-900 dark:text-zinc-100">{i + 1}</span>
            ...
            {days.map((_date) => {
              return (
                <div
                  key={_date.toString()}
                  className={cn(
                    getMonth(_date) !== i && "text-zinc-300 dark:text-zinc-600",
                  )}
                >
                  <div
                    className={cn(
                      ...
                      isSameDay(today, _date) &&
                        getMonth(_date) === i &&
                        "bg-indigo-600 text-white dark:bg-indigo-500 rounded-full",
                    )}
                  >
                    {format(_date, "d")}
                  </div>
  ...
  return (
    <div className="pr-2 w-12">
      {Array.from(Array(25).keys()).map((hour) => {
        return (
          <div
            className="text-right relative text-xs text-zinc-400 dark:text-zinc-500 h-20 last:h-0"
            key={hour}
          >
            {now.getHours() === hour && (
              <div
                className="absolute z-10 left-full translate-x-2 w-dvw h-0.5 bg-red-500"
                style={{
