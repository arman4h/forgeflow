...
export const SpaceDetailView: React.FC = () => {
  ...
  const spaceSettings = getSpaceSettings(currentSpace.id);
  const isOwner = myRole === 'owner';
  const isManager = myRole === 'manager' || isOwner;
  const canCreateTask = isManager || spaceSettings.members_can_create_tasks;
  const canInvite = isManager || spaceSettings.who_can_invite === 'everyone';
  ...
  const getLatestComment = (taskId: string) => {
    ...
    >
      ...
      <div className="bg-white dark:bg-black rounded-lg border border-zinc-200 dark:border-zinc-800 p-5 md:p-6 shadow-2xs">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="flex items-start gap-4">
            ...
            <div>
              ...
              {currentSpace.description ? (
                <MarkdownContent content={currentSpace.description} className="mt-1 max-w-2xl" />
              ) : (
                <p className="text-xs text-zinc-500 dark:text-zinc-400 mt-1 max-w-2xl leading-relaxed">
                  Collaborative workspace for this project.
                </p>
              )}
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {canCreateTask && (
            <Button
              onClick={() => { setCreateTaskDefaultSpaceId(currentSpace?.id || null); setIsCreateTaskOpen(true); }}
              variant="default"
              size="sm"
              className="flex items-center gap-1.5"
            >
      ...
      {selectedSpaceTab === 'overview' && (
        <div className="space-y-6">
          ...
          {/* Project Description */}
          {(isManager || currentSpace.description) && (
            <div className="p-5 rounded-lg bg-white dark:bg-black border border-zinc-200 dark:border-zinc-800 shadow-2xs">
              ...
              {editingDescription ? (
                <div className="space-y-3">
                  ...
                  <div className="flex items-center gap-2">
                    <Button
                      size="sm"
                      onClick={() => {
                        updateSpace(currentSpace.id, { description: descriptionDraft.trim() || undefined });
                    ...
                    <Button
                      ...
                      variant="outline"
                      onClick={() => {
                        setDescriptionDraft(currentSpace.description || '');
                        setEditingDescription(false);
                      }}
                    >
                      Cancel
                ...
                <MarkdownContent content={currentSpace.description} />
              ) : (
                      ...
                      <div className="flex items-center gap-3 min-w-0">
                        ...
                        >
                          ...
                          <div className="flex items-center gap-3 mt-1 text-[11px] text-zinc-500 dark:text-zinc-400">
                            ...
                            {getLatestComment(task.id) && (
                              <span className="flex items-center gap-1 text-zinc-400 dark:text-zinc-500">
                                <MessageSquare className="w-3 h-3" />
                                <span className="truncate max-w-[140px]">{getLatestComment(task.id)!.content}</span>
                              </span>
                            )}
                          </div>
                        </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {(
                ...
                <div
                  ...
                  onDragOver={(e) => {
                    e.preventDefault();
                    e.currentTarget.classList.add('border-cyan-400', 'bg-cyan-50/50');
                  }}
                  onDragLeave={(e) => {
                    e.currentTarget.classList.remove('border-cyan-400', 'bg-cyan-50/50');
                  }}
                  onDrop={(e) => {
                    e.preventDefault();
                    e.currentTarget.classList.remove('border-cyan-400', 'bg-cyan-50/50');
                    ...
                    if (!task) return;
                    const canChange = isManager || task.assigneeId === currentUser.id;
                    if (!canChange) return;
                    moveTaskStatus(taskId, col.key as TaskStatus);
                  }}
                  ...
                  <div className="space-y-2 min-h-[200px]">
                    {col.items.map(task => {
                      ...
                      return (
                        <div
                          key={task.id}
                          draggable={isManager || task.assigneeId === currentUser.id}
                          onDragStart={(e) => {
                        ...
                        >
                          ...
                          {getLatestComment(task.id) && (
                            <div className="flex items-center gap-1 text-[10px] text-zinc-400 dark:text-zinc-500 pt-1">
                              <MessageSquare className="w-3 h-3 flex-shrink-0" />
                              <span className="truncate">{getLatestComment(task.id)!.content}</span>
                            </div>
                          )}
              ...
              milestonesList.map(m => (
                ...
                >
                  <div className="space-y-1">
                    ...
                    {m.description && (
                      <MarkdownContent content={m.description} className="mt-1" />
                    )}
                    {m.targetDeliverable && (
      ...
      {/* TAB 6: CALENDAR */}
      {selectedSpaceTab === 'calendar' && (
        <div className="bg-white dark:bg-black rounded-lg border border-zinc-200 dark:border-zinc-800 p-6 space-y-4">
          <div className="flex items-center justify-between pb-4 border-b border-zinc-100 dark:border-zinc-800">
            <div>
              ...
              <p className="text-xs text-zinc-500 dark:text-zinc-400">
                All tasks and milestones with due dates in {currentSpace.name}
              </p>
            </div>
          </div>
              ...
              events={[
                ...spaceTasks
                  ...
                  .map(t => ({
                    ...
                    color: (t.status === 'done' ? 'green' : t.priority === 'urgent' ? 'pink' : t.priority === 'high' ? 'blue' : 'default') as 'default' | 'blue' | 'green' | 'pink' | 'purple',
                  })),
                ...milestonesList
                  .filter(m => m.dueDate)
                  .map(m => ({
                    ...
                    color: 'purple' as 'default' | 'blue' | 'green' | 'pink' | 'purple',
                  })),
              ]}
              onEventClick={event => {
                if (event.title.startsWith('🏁')) {
                  setSelectedSpaceTab('milestones');
                } else {
      ...
      {/* TAB 10: ACCESS */}
      {selectedSpaceTab === 'access' && <AccessTab />}
    </div>
  );
};
