...
export const CreateTaskModal: React.FC = () => {
  ...
  const initialSpaceId = createTaskDefaultSpaceId || currentSpace?.id || personalSpace.id;
  const isSpaceLocked = !!createTaskDefaultSpaceId;
  ...
  useEffect(() => {
    if (!isCreateTaskOpen || isTargetPersonal) {
      setMemberOptions([]);
      return;
    }

    const ctxMembers = getSpaceMembers(targetSpaceId);
    ...
    let cancelled = false;
    api.getSpaceMembers(targetSpaceId).then(members => {
      if (cancelled) return;
      setMemberOptions(
        members.map(m => ({
          userId: m.userId,
  ...
  }, [isCreateTaskOpen, targetSpaceId, isTargetPersonal]);

  useEffect(() => {
    if (isCreateTaskOpen) {
      setSpaceId(createTaskDefaultSpaceId || currentSpace?.id || personalSpace.id);
  ...
  const handleClose = () => {
    ...
    setCreateTaskDefaultSpaceId(null);
    setTitle('');
    setDescription('');
    setDueDate('');
    setChecklistItems([]);
