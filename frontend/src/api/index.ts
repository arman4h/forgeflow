...
import type {
  User, Space, SpaceMember, SpaceSettings, Task, Note,
...
export const addSpaceMember = (spaceId: string, userId: string, role?: string, callerUserId?: string) =>
  api.post<SpaceMember>(`/spaces/${spaceId}/members`, { userId, role, _userId: callerUserId });
export const removeSpaceMember = (spaceId: string, userId: string) =>
  api.delete<void>(`/spaces/${spaceId}/members/${userId}`);
export const updateMemberRole = (spaceId: string, userId: string, role: string, callerUserId: string) =>
  api.put<SpaceMember>(`/spaces/${spaceId}/members/${userId}/role`, { role, _userId: callerUserId });

// Space Settings
export const getSpaceSettings = (spaceId: string) =>
  api.get<SpaceSettings>(`/spaces/${spaceId}/settings`);
export const updateSpaceSettings = (spaceId: string, settings: Partial<SpaceSettings>, callerUserId: string) =>
  api.put<SpaceSettings>(`/spaces/${spaceId}/settings`, { ...settings, _userId: callerUserId });
...
export const updateTask = (id: string, data: Partial<Task> & { _userId?: string }) =>
  api.put<Task>(`/tasks/${id}`, data);
export const deleteTask = (id: string, userId?: string) =>
  api.delete<void>(`/tasks/${id}${userId ? `?userId=${userId}` : ''}`);
export const moveTaskStatus = (id: string, status: TaskStatus, userId: string) =>
  api.patch<Task>(`/tasks/${id}/status`, { status, userId });
